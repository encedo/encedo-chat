 ...
 * direct DataChannel — then swap a "hello". No HEM, no our message crypto: this
 * just proves browser↔browser WebRTC works on our relay.
 */
...
function startLink(peer: string, initiator: boolean) {
  ...
  link = webrtcLink({
    initiator,
    sendSignal: (sig) => pub({ t: 'rtc', from: me, to: peer, sig }),
    onData: (bytes) => log(`✅ RECV over DataChannel: "${dec.decode(bytes)}"`),
    onOpen: () => {
      log('✅✅ DataChannel OPEN — direct P2P established')
...
const hb = setInterval(() => { if (!link) pub({ t: 'hello', from: me }) }, 2000)
setTimeout(() => clearInterval(hb), 60_000)
