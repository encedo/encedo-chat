...
const CATALOG: Record<string, Record<string, Entry>> = {
  ...
  en: {
    ...
    'Zaproszenia': 'Invites',
    'CZEKAM': 'WAITING',
    'Zapukaliśmy — czekamy, aż {name} przyjmie': 'Knocked — waiting for {name} to accept',
    ...
    'Prośby nie są zapisywane na dysku. Jeśli zamkniesz aplikację, druga strona zapuka ponownie.':
      'Requests are not stored on disk. Close the app and the other side will knock again.',
    'Ktoś puka — przyjmij dopiero po sprawdzeniu odcisku': 'Somebody is knocking — accept only after checking the fingerprint',
    'Ktoś puka do zaproszenia „{label}"': 'Somebody is knocking at “{label}”',
    'odcisk: ': 'fingerprint: ',
    'przez: ': 'through: ',
    'Bez nazwy': 'No name given',
    'Przyjmij': 'Accept',
    'Zignoruj': 'Ignore',
    'Wycofaj': 'Retire',
    'Wycofać „{label}"?': 'Retire “{label}”?',
    'Przestaniesz słuchać na tym zaproszeniu. Kto ma ten link, nie dopuka się już nigdy. Pozostałe zaproszenia działają dalej.':
      'You stop listening on this invite. Whoever holds that link can never knock again. Your other invites keep working.',
    'Dodano kontakt „{name}"': 'Added contact “{name}”',
    'Nie udało się skopiować': 'Could not copy',
    'Skopiowano': 'Copied',
    'Najpierw się zaloguj': 'Sign in first',
    ...
    'Dziennik połączenia': 'Connection log',
    'Zapisuje, co dzieje się z łącznością — widoczność okna, stan przekaźnika, pojawianie się i znikanie kontaktów — z zegarem i po restarcie. Nie zapisuje treści rozmów.': 'Records what happens to connectivity - the window being hidden, the relay, contacts appearing and going quiet - with a clock, and across a restart. It records nothing anybody said.',
    'Skopiuj dziennik': 'Copy the log',
    'Dziennik skopiowany ({n} linii)': 'Log copied ({n} lines)',
    'Schowek odmówił — dziennik jest w konsoli': 'The clipboard refused - the log is in the console',
    'zostało {n}': '{n} left',
    ...
    '{d}, {t}': '{d}, {t}',
    'nigdy się nie odezwał': 'never answered',
    'Jeszcze się nie odezwał — jeśli nie ma Twojego klucza, wyślij mu swój kod (kliknij)': 'Has not answered yet - if they do not have your key, send them your code (click)',
    'Ani razu się nie odezwał. Albo go nie było, albo nie ma Twojego klucza — kliknij, żeby wysłać kod ponownie': 'Has never once answered. Either they were away, or they do not have your key - click to send your code again',
    'Dodano {name}': '{name} added',
    '{name} nie ma jeszcze Twojego klucza — bez niego nie zobaczycie się nawzajem. Odesłać teraz swój kod?': '{name} does not have your key yet - without it you cannot see each other. Send them your code now?',
    'Odeślij mój kod': 'Send my code',
    'Powiększ pole': 'Make the box taller',
    'Zmniejsz pole': 'Make the box shorter',
    ...
    'czekam na potwierdzenie…': 'waiting for them to accept…',
    '{s} s': '{s} s',
    'zostało ~{s} s': '~{s} s left',
    'Przychodzi plik': 'Incoming file',
    '{who} chce wysłać:': '{who} wants to send:',
    'Transfer bezpośredni — plik idzie prosto z tamtej przeglądarki do Twojej, nie przez nasz serwer. Musicie oboje zostać w rozmowie do końca.': 'Direct transfer — the file comes straight from their browser to yours, not through our server. You both have to stay in the conversation until it finishes.',
    'Transfer: {name} — wysłany': 'Transfer: {name} — sent',
    'Odebrano': 'Received',
    'Plik jest tylko w tej karcie. Zamknięcie jej znaczy, że trzeba go wysłać jeszcze raz.': 'The file is only in this tab. Closing it means sending it again.',
    'Transfer: {name} — odebrany': 'Transfer: {name} — received',
    'Transfer: {name} — {why}': 'Transfer: {name} — {why}',
    'Transfer: {why}': 'Transfer: {why}',
    'Transfer przerwany': 'Transfer stopped',
    'Kanał bezpośredni przestał działać. Plik nie doszedł — wyślij go przez czat.': 'The direct channel stopped working. The file did not arrive — send it through the chat.',
    'Odbieram': 'Receiving',
    'Plik idzie prosto z tamtej przeglądarki do Twojej.': 'The file is coming straight from their browser to yours.',
    'odrzucony': 'refused',
    'niepodjęty': 'never picked up',
    'przerwany': 'stopped',
    'przerwany po drugiej stronie': 'stopped by the other side',
    'kanał bezpośredni padł': 'the direct channel died',
    'druga strona jest zajęta': 'the other side is busy',
    'za duży': 'too big',
    'pusty': 'empty',
    'uszkodzony w drodze': 'damaged on the way',
    'Przerwij': 'Stop',
    'Odbierz': 'Receive',
    'od: {who}': 'from: {who}',
    'Wysłano': 'Sent',
    '🔒 Tylko bezpośrednio': '🔒 Direct only',
    '🔒 Tylko bezpośrednio — węzeł nie zobaczy nawet szyfrogramu': '🔒 Direct only — the node will not even see ciphertext',
    'Nowe rozmowy pójdą wyłącznie kanałem bezpośrednim — bez grup': 'New conversations will use the direct channel only — no groups',
    'Ta przeglądarka nie ma WebRTC — nie ma czym prowadzić rozmowy bez węzła': 'This browser has no WebRTC — there is nothing to carry a conversation without the node',
    'Grupy nie działają w trybie „tylko bezpośrednio” — zmień transport w Ustawieniach': 'Groups do not work in direct-only mode — change the transport in Settings',
    'Bezpośredni kanał jest szybszy i nie przechodzi przez węzeł, ale rozmówca widzi wtedy Twój adres IP. Wybór dotyczy rozmów otwieranych od teraz — trwającej nie przełączamy, bo to zerwałoby kanał w środku zdania. Odznaka w nagłówku pokazuje stan faktyczny każdej rozmowy. Tryb „tylko bezpośrednio” idzie dalej: węzeł służy wtedy wyłącznie do spotkania i uzgodnienia klucza, a wiadomość bez kanału czeka z możliwością ponowienia, zamiast pójść przez węzeł. Grupy w tym trybie nie działają — broadcast do wielu osób nie ma kanału bezpośredniego.': 'A direct channel is faster and does not pass through the node, but the other side then sees your IP address. The choice applies to conversations opened from now on — a live one is not switched, because that would cut the channel mid-sentence. The badge in the header shows what each conversation is actually doing. Direct-only goes further: the node is then used only to meet and agree a key, and a message with no channel waits with a re-send button rather than taking the node. Groups do not work in this mode — a broadcast to several people has no direct channel.',
    'bezpośrednio': 'direct',
    ...
    'Za słabe hasło — miernik musi być pełny. Najprościej dopisać drugie słowo albo wydłużyć do 12+ znaków. Profilu nie da się odzyskać, więc to hasło jest całą jego ochroną.': 'Too weak — the meter has to be full. The easiest fix is a second word, or 12+ characters. The profile cannot be recovered, so this password is all the protection it has.',
    ...
    // feedback form
    'Zgłoś błąd, pomysł lub pytanie': 'Report a bug, an idea or a question',
    'Zgłoś uwagę': 'Send feedback',
    'Błąd, pomysł, pytanie — kilka zdań wystarczy. Bez konta, bez logowania.': 'A bug, an idea, a question — a few sentences are enough. No account, no sign-in.',
    'Usterka': 'Bug',
    'Pomysł': 'Idea',
    'Pytanie': 'Question',
    'Pytanie o HEM': 'About HEM',
    'Co się stało / co byś zmienił(a)?': 'What happened / what would you change?',
    'O co chcesz zapytać? Do czego chcesz używać HEM?': 'What would you like to ask? What would you use a HEM for?',
    'Kontakt (opcjonalnie — jeśli chcesz odpowiedzi)': 'Contact (optional — if you want a reply)',
    'e-mail lub handle': 'email or handle',
    'Dołącz dane techniczne (wersja, platforma, diagnostyka)': 'Include technical details (version, platform, diagnostics)',
    'Nie wysyłamy nic o Twojej tożsamości, kluczach ani rozmowach. Serwer onchato.com zobaczy Twój adres IP i godzinę — jak przy każdym żądaniu.': 'Nothing about your identity, keys or conversations is sent. The onchato.com server will see your IP address and the time — as with any request.',
    'Pokaż, co dokładnie wyślę': 'Show exactly what will be sent',
    'Ukryj': 'Hide',
    'Kopiuj zgłoszenie': 'Copy the report',
    'Zgłoszenie skopiowane': 'Report copied',
    'Napisz choć zdanie.': 'Write at least a sentence.',
    'Za dużo zgłoszeń naraz — spróbuj za chwilę.': 'Too many reports at once — try again in a moment.',
    'Nie udało się wysłać — sieć? Skopiuj zgłoszenie i prześlij je nam inną drogą.': 'Could not send — no network? Copy the report and send it to us another way.',
    'Dziękujemy — zgłoszenie wysłane ✓': 'Thank you — sent ✓',
    'Nie masz HEM? Zapytaj, co daje klucz sprzętowy →': 'No HEM? Ask what a hardware key gives you →',
    'Ten profil trzyma klucz w przeglądarce. W HEM klucz nigdy nie opuszcza urządzenia —': 'This profile keeps its key in the browser. In a HEM the key never leaves the device —',
    'zapytaj o HEM →': 'ask about HEM →',
  },
}
