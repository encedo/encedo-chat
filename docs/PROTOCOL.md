...
**Storage framing — 8 bytes in front of the ciphertext.** What the store holds is `"ECF1" ‖ u8(version=1) ‖ 0x000000 ‖ ciphertext`, and it is framing, not crypto: no key, no length, nothing the AEAD above depends on, and the manifest still describes the blob exactly as before. It exists because the upload endpoint is open by design (an authenticated one would learn who sent what and when — §12), so the store needs some way to tell a file of ours from an arbitrary document somebody uploads to be served from our address: a guard can refuse a body that does not begin with those bytes, and whatever does begin with them is answered as `application/octet-stream` rather than rendered. A reader that finds the magic with an **unknown version** refuses rather than decrypting bytes laid out differently. The prefix is constant across releases on purpose — keyed to a build, a file sent from one version would not open in the next.
