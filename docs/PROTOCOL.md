...
**The node list** ships **compiled into the client** (`infra/nodes.json`) and can be refreshed at runtime by fetching a **compiled-in IPFS CID** — content addressing is the whole of the list's integrity (a new list means a new build); there is no signature on it. The client draws its first node among the enabled ones (weighted by capacity, uniform without weights), may move once at login to a node announcing a load at least 20 points lower (the nodes' own load announcements on the load topic, replayed at once by the node reached), and fails over down the rest of the list; the user can disable nodes or override the list locally. Full node-operations detail in `relay/README.md` and `ARCHITECTURE.md`.
...
**Storage framing — 8 bytes in front of the ciphertext.** What the store holds is `"ECF1" ‖ u8(version=1) ‖ 0x000000 ‖ ciphertext`, and it is framing, not crypto: no key, no length, nothing the AEAD above depends on, and the manifest still describes the blob exactly as before. **The operator's upload endpoint writes it, not the client** — every upload gets it, including one that never came from this application — and nothing on the way out removes it. It exists because that endpoint is open by design (an authenticated one would learn who sent what and when — §12), and the consequence was that arbitrary content could be served from the operator's own name: what enters the store is now never what leaves it, so the store cannot be used to hand anyone a byte-identical file, and content sniffing answers `application/octet-stream` rather than rendering a page. A reader that finds the magic with an **unknown version** refuses rather than decrypting bytes laid out differently. The prefix is constant across releases on purpose — keyed to a build, a file sent from one version would not open in the next.
...
**A file can also skip the store entirely** when the direct plane is carrying content: §13.1 sends it as frames on the DataChannel, so none of the paragraphs above apply to it — no CID, no operator, no lease. That path is 1:1, browser-only and opt-in with the plane; everything else still goes the way described here.
...
**The discovery node** additionally sees, for traffic it carries: which topics are active, frame sizes and timing, and — on the direct plane — that a pair is negotiating WebRTC (the `rtc` envelope is sealed, so not the addresses). **The STUN server** (§13) sees a client's IP and negotiation timing on every direct attempt — since 2026-09-03 that is the operator's own node rather than a public third party, so it is an address the operator already holds; and direct is opt-in, so a client that has not chosen it consults nothing. **The file store (§7.5, S11)** sees blob sizes, fetch timing and IPs for a blob's ~5-minute life. **The MQTT fall-back transport**, where an operator deploys it, concedes more: any connected client can subscribe to the wildcard and observe every room's activity/timing/size (a static broker ACL cannot scope reads to runtime-secret rooms) — content stays E2E; documented in the README as a reach-over-privacy trade. **GossipSub narrows this rather than removing it, and the difference is smaller than it first appears** — see below.


**Any peer connected to a discovery node** — not merely its operator — learns the node's whole set of active topics, and this is a property of GossipSub rather than of this deployment. A node announces its subscriptions to every peer it is connected to; that is precisely how a node learns what its own clients want (`relay.mjs`, `subscription-change`). A relay carrying rooms therefore publishes their topic ids to anybody who dials it, and dialling is open by construction: clients are peers with ephemeral ids, so there is nobody to admit and nobody to turn away. The observer can then subscribe to those ids and watch each room's activity, sizes and timing — **content stays E2E, and the topic still does not reveal the pair** (topic→pair needs both IK_pubs), but "unguessable" protects only against enumeration by search, and the ids are not obtained by searching. Compared with the MQTT fall-back above, what GossipSub removes is the convenience of one wildcard, not the visibility. Two things bound it: `--local-topics-only` (2026-09-23) means a node carries only its own clients' topics rather than the network's, so an observer sees one node's share instead of everything; and `--max-topics-per-peer` caps how many a single peer may hold at once. Neither is a fix, and the fix — announcing to each peer only the topics it already holds — means a fork of the GossipSub implementation. **This is why the direct plane (§13) exists and why nothing here is logged**: an open relay sees metadata by construction, so the protocol's answer is to carry less of it and to keep none of it.


**The client is a light client by default (since 0.6.24): `/onchato/pick/1`.** It opens one stream to its node and names topics instead of joining GossipSub: PICK/DROP a topic (answered PICKED, or REFUSED when the node is full — the first refusal in the system that is said rather than silent), PUSH bytes onto a topic (the node publishes them unchanged into GossipSub and answers ACK with the reach), and DELIVER frames back. GossipSub runs only between nodes; `?light=0` restores the full GossipSub peer. **This changes what the node CARRIES for the client, not what it KNOWS**: a picked topic is recorded per peer exactly as a subscription is. What disappears for the client is mesh upkeep, subscription lists, gossip and per-frame signatures. Since 0.6.22 the sender travels **inside every frame** (the origin envelope, §13), so a relay publishing on a client's behalf (stage 2, `push`) no longer attributes the message to itself — the receiver never reads the transport's word about who published. What the envelope does NOT do is authenticate: `from` is the key under which a receiver files state (which EH-2 session, which handshake, which ratchet), not evidence of who is speaking. It never was — a libp2p PeerId is generated by whoever shows up, and StrictSign only ever proved that the holder of that throwaway key sent the packet. Identity is proven by EH-2 (`initiator_id`, the IK-derived DHs) and message integrity by the ratchet's AEAD and the group MAC; a forged `from` files a frame under the wrong session, where it does not open. Nothing in the client may treat `from` as identity, and the audit question to ask of any new code is exactly that.
...
|---|---|---|
| Control | rendezvous subscriptions, Announce, EH-2 frames, `rtc` signalling, group keepalive | client ↔ node: `/onchato/pick/1` (PICK/PUSH; the default since 0.6.24, a full GossipSub peer with `?light=0`) · node ↔ node: GossipSub |
...
**Origin envelope (normative since 0.6.21).** Every frame a client publishes on a **pair, self or group topic** is wrapped:

```
0xE1 ‖ len(u8) ‖ from (UTF-8, len bytes) ‖ frame
```

`from` is the publisher's own PeerId string today, and is opaque to the protocol — a client that is not a libp2p peer may put any per-session identifier there. A receiver takes `from` and `frame` from the envelope when the first byte is `0xE1`, and from the transport (`from` = the GossipSub publisher, `frame` = the bytes as received) otherwise; a tagged frame whose length does not fit is dropped, never attributed to the transport. `0xE1` collides with no frame family: EH-2 `0x01`–`0x03`, ratchet content `0x10`, group `0x20`/`0x21`, Announce `0x7B` (`{`). **Inbox topics are excluded**: a knock (§5.8) is published bare, because its first 32 bytes are a random ephemeral key that may be anything, because it uses no `from`, and because a decoy must remain indistinguishable from a real knock — a readable sender on the outside would break §5.9. The purpose is §12's: with the sender inside the frame, who *published* the GossipSub message stops mattering, which is what lets a relay publish for a light client (`push`). Cost: 2 + len bytes per frame (~54 B with a PeerId). `from` is a state key, not an identity claim — see §12.

- **Direct is the P1 posture** (peers see each other's IPs) and, since 2026-09-03, it is **opt-in rather than the default**. A client sends content through the node unless the user selects the direct plane in Settings; the switch is per-device, applies to conversations opened from that point, and works one-sidedly — a channel needs both ends, so one side declining is enough. The reason is behavioural, not cryptographic: the direct path is faster where it works and fails asymmetrically where it does not (a DataChannel that opens and then swallows traffic, a NAT pair that never completes, a webview with no `RTCPeerConnection` at all — §3.2), while the node path behaves the same everywhere. Two exposures go with the default: the peer no longer learns the device's address, and no STUN server is consulted at all. When direct IS selected, ICE learns its reflexive address from **the operator's own STUN servers, which are the discovery nodes themselves**: a node answers Binding Requests on UDP 3478 as part of being a node, so a client asks the hosts it already dials (`stun:<node>:3478`, derived from its node list — there is no separate server list to configure or to drift) and stops at the first answer. The server is `infra/stun/stun.mjs`: Binding Requests only, no TURN, no relaying, no RFC 5780 behaviour discovery. Until 2026-09-03 this was Google's public server, which meant a third party outside the operator saw a client's IP and negotiation timing on every attempt; it is nobody's now — the nodes asked are the ones already carrying that client's WSS connection, so the answer tells the operator nothing it did not have. A different server can still be pointed at for a page load (`?stun=`), and no STUN at all (`?stun=0`) leaves ICE with host candidates, which is what a pair on one LAN uses anyway. What rides GossipSub — presence, signalling, handshake frames, node-path and group content — is ciphertext + metadata to the relay. A **relay-blind plane** (circuit-relay-v2 streams: no IP exposure to the node either, opaque NOISE bytes, Tor-compatible) is **designed but parked**: on the pinned libp2p 2.2.x generation circuit-relay-v2's destination-side STOP handling is broken, and the v3 upgrade is blocked until GossipSub ships a libp2p-v3-compatible release. Note the two are different promises: today's node-only default hides the address from the PEER, while the parked plane would hide it from the NODE. The Linux desktop is node-only by platform limitation (§3.2) whatever the setting says.
### 13.1 Direct file transfer

A file may travel **on the DataChannel itself**, as its own frames, with no store involved: no upload, no CID, no five-minute lease, nothing an operator can fetch afterwards. It is offered **only while the direct plane is actually carrying content** and never falls back to the node — measured, one room through a node sustains about 1 MB/s with everything delivered, so a node-borne transfer would be a slower copy of §7.5 paid for in the operator's bandwidth. 1:1 only (groups have no acknowledgements and would need N channels), and absent on the Linux desktop for the same reason the plane itself is (§3.2).

**Frames** share the channel's control prefix `0x00` — content frames begin at `0x10`, so the two cannot collide — followed by a subtype and a 32-bit transfer id: `0x01` offer, `0x02` accept, `0x03` reject, `0x04` chunk, `0x05` receipt, `0x06` cancel, `0x07` done. `0x50`/`0x4f` remain the channel's own liveness ping and pong. The offer carries `{name, size, mime, chunk, chunks}`; a receiver that computes a different geometry from `size` refuses it rather than negotiating. Chunks are **64 KiB**, in order, each `0x04 ‖ id ‖ u32be(index) ‖ bytes`.

**Reliability is the channel's, not ours.** The DataChannel is reliable and ordered, so there is no retransmission, no window and no gap repair in these frames — a chunk arriving out of order means the channel is not what it claims to be, and the transfer **fails** rather than reassembling something nobody can vouch for. Receipts are therefore **progress, not delivery**: one every `max(1, ceil(chunks/100))` chunks, so between fifty and a hundred per transfer whatever the size, derived identically on both sides from the offer. Losing one costs a stale percentage.

**Consent and limits.** The receiving side is asked before any byte moves, because the transfer is synchronous: an offer unanswered for **30 s** expires, silence mid-transfer for **60 s** fails it, and either side may cancel at any point — the peer is told in every case. **512 MiB** is the ceiling and it is a memory limit, not a policy one: the receiver holds the file until it is saved. One transfer at a time per conversation; a second offer is answered with a refusal rather than queued.

**What it does not change:** nothing here touches §7.5. The store path remains for groups, for browsers without WebRTC, for the packaged desktop, and for every conversation where the direct plane is not in use.
...
| Frame type bytes | `0x01`–`0x03` EH-2 msg1–3 · `0x10` 1:1 ratchet content · `0x20` group message · `0x21` group keepalive · `0xE1` origin envelope (§13, outermost on pair/self/group topics; never on inbox topics) |
...
**Limitations (by design):** relay metadata is visible to any peer that dials a discovery node, not only to its operator (§12) — the mitigation is to carry less and keep none, not to restrict who may connect; no offline messages; no message-history portability across devices (the §10 export moves identity + contacts + groups, never a transcript); out-of-band trust establishment (no built-in key transparency); clock dependency (±5 min, absorbed by the §5.4 guard); single active session (no simultaneous laptop+phone).
...
├── lib/        rendezvous · announce · origin (sender-in-frame envelope, §13) · presence · room (delivery contract, EH-2 seam)
