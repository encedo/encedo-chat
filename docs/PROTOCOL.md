...
**A light client (`/onchato/pick/1`, stage 1 of the light-client plan) changes what the relay CARRIES for that client, not what it KNOWS.** A picked topic is recorded per peer exactly as a GossipSub subscription is, so the relay's view of "which peer wants which topic" is unchanged; what disappears is the client's mesh membership, its subscription announcements and the relay's announcements to it. Two things do change, both in the client's favour: the relay hands on the GossipSub publisher id with each delivery (sessions in `lib/room.ts` are keyed by it), and a topic refused at the cap is now REFUSED to the client's face instead of silently never forming. Since 0.6.21 the sender travels **inside every frame** (the origin envelope, §13), so a relay publishing on a client's behalf (stage 2, `push`) no longer attributes the message to itself — the receiver never reads the transport's word about who published. What the envelope does NOT do is authenticate: `from` is the key under which a receiver files state (which EH-2 session, which handshake, which ratchet), not evidence of who is speaking. It never was — a libp2p PeerId is generated by whoever shows up, and StrictSign only ever proved that the holder of that throwaway key sent the packet. Identity is proven by EH-2 (`initiator_id`, the IK-derived DHs) and message integrity by the ratchet's AEAD and the group MAC; a forged `from` files a frame under the wrong session, where it does not open. Nothing in the client may treat `from` as identity, and the audit question to ask of any new code is exactly that.
